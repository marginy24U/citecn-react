...
const Home = () => {
    ...
    useEffect(() => {
        const getProductos = async () => {
            setLoading(true);
            try {
                const { data } = await axios.get(`https://regxipruebas.cloud/API/productos/all`);
    ...
    const guardarP = async () => {
        try {
            const { data } = await axios.post(`https://regxipruebas.cloud/API/productos/`, producto);
    ...
    const ActualizaP = async () => {
        try {
            const { data } = await axios.put(`https://regxipruebas.cloud/API/productos/${id_producto}`, producto);
    ...
    const deleteProd = async (id) => {
        try {
            const { data } = await axios.put(`https://regxipruebas.cloud/API/productos/del/${id}`);
            // const { data } = await axios.put(`http://localhost:3050/api/productos/del/${id}`);
            if (data.status != 200) {
                Swal.fire({
                    icon: 'error',
                    title: data.message,
