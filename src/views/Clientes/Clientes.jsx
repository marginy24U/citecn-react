...
const Clientes = () => {
    ...
    const guardarC = async () => {
        try {
            const { data } = await axios.post(`https://regxipruebas.cloud/API/clientes/`, cliente);
    ...
    const ActualizaC = async () => {
        try {
            const { data } = await axios.put(`https://regxipruebas.cloud/API/clientes/${id_cliente}`, cliente);
    ...
    const deleteC = async (id) => {
        try {
            const { data } = await axios.put(`https://regxipruebas.cloud/API/clientes/del/${id}`);
            // const { data } = await axios.put(`http://localhost:3050/api/clientes/del/${id}`);
            if (data.status != 200) {
                Swal.fire({
                    icon: 'error',
                    title: data.message,
