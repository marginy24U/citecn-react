...
const Materiales = () => {
    ...
    useEffect(() => {
        const getMateria = async () => {
            setLoading(true);
            try {
                const { data } = await axios.get(`https://regxipruebas.cloud/API/materia/all`);
    ...
    const guardarM = async () => {
        try {
            const { data } = await axios.post(`https://regxipruebas.cloud/API/materia/`, materia);
    ...
    const ActualizaM = async () => {
        try {
            const { data } = await axios.put(`https://regxipruebas.cloud/API/materia/${id_materia}`, materia);
    ...
    const deleteM = async (id) => {
        try {
            const { data } = await axios.put(`https://regxipruebas.cloud/API/materia/del/${id}`);
            // const { data } = await axios.put(`http://localhost:3050/api/materia/del/${id}`);
            if (data.status != 200) {
                Swal.fire({
                    icon: 'error',
                    title: data.message,
