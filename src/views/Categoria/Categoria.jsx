...
const Categoria = () => {
    ...
    const guardarCat = async () => {
        try {
            // const { data } = await axios.post(`https://regxipruebas.cloud/API/categorias`, categoria);
    ...
    const ActualizaCat = async () => {
        try {
            const { data } = await axios.put(`https://regxipruebas.cloud/API/categorias/${id_categoria}`, categoria);
    ...
    const deleteCat = async (id) => {
        try {
            const { data } = await axios.put(`https://regxipruebas.cloud/API/categorias/del/${id}`);
            // const { data } = await axios.put(`http://localhost:3050/api/categorias/del/${id}`);
            if (data.status != 200) {
                Swal.fire({
                    icon: 'error',
                    title: data.message,
