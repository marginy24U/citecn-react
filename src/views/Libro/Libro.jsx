...
import TablaCat from './components/TablaCat';
...
const Libro = () => {
    ...
    const formFill = (title, desc, st, min, pr, ub, ic, nc) => {
        setLibro({
    ...
    useEffect(() => {
        const getLibro = async () => {
            setLoading(true);
            try {
                const { data } = await axios.get(`https://regxipruebas.cloud/API/libro/all`);
    ...
    const guardarL = async () => {
        try {
            const { data } = await axios.post(`https://regxipruebas.cloud/API/libro/`, libro);
    ...
    const ActualizaL = async () => {
        try {
            const { data } = await axios.put(`https://regxipruebas.cloud/API/libro/${id_libro}`, libro);
    ...
    const deleteL = async (id) => {
        try {
            const { data } = await axios.put(`https://regxipruebas.cloud/API/libro/del/${id}`);
            // const { data } = await axios.put(`http://localhost:3050/api/libro/del/${id}`);
            if (data.status != 200) {
                Swal.fire({
                    icon: 'error',
                    title: data.message,
            ...
            <div className={`contenido ${!mOpen && "desplegado"}`}>
                ...
                <div className="card mt-4 mx-auto text-center">
                    ...
                    <div className="card-body">
                        ...
                        <div className="modal fade" id="modalAddEL" aria-labelledby="exampleModalLabel" aria-hidden="true" tabindex="-1">
                            <div className="modal-dialog modal-dialog-centered">
                                <div className="modal-content">
                                    <div onClick={()=>console.log(nombreCat)} className="modal-header">
                                        {
                                            isEditar
                                                ?
                                    ...
                                    <div className="modal-body">
                                        ...
                                        <FrmActions libro={libro} setLibro={setLibro} nombreCat={nombreCat} setNombreCat={setNombreCat} guardarL={guardarL} ActualizaL={ActualizaL} isEditar={isEditar} />
                                    </div>
                        ...
                        <div className="modal fade" id="modalSelCat" aria-hidden="true" aria-labelledby="exampleModalToggleLabel2" tabindex="-1">
                            <div className="modal-dialog modal-dialog-centered">
                                <div className="modal-content">
                                    <div className="modal-header">
                                        <h5 className="modal-title" id="exampleModalToggleLabel2">Selecciona una Categoría</h5>
                                        <button type="button" className="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                                    </div>
                                    <div className="modal-body">
                                        <TablaCat libro={libro} setLibro={setLibro} nombreCat={nombreCat} setNombreCat={setNombreCat} />
                                    </div> 
                                    <div className="modal-footer">
