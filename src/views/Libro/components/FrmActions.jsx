...
const FrmActions = ({ libro, setLibro, nombreCat, setNombreCat, guardarL, ActualizaL, isEditar }) => {
    ...
    return (
        <form onSubmit={(e) => { e.preventDefault(); validaCampos(); e.target.reset(); }} className="row g-3 needs-validation" novalidate>
            ...
            <div className="col-md-6 flex flex-md-column">
                <label className="form-label"><span className="text-danger">*</span>Categoria</label>
                <div className="input-group ">
                    <span data-bs-toggle="modal" data-bs-target="#modalSelCat" data-bs-dismiss="modal" className="input-group-text bg-success text-white"><i className="fas fa-plus-circle"></i></span>
                    <input name="" value={nombreCat} type="text" className="form-control" readOnly required />
                </div>
            </div>
