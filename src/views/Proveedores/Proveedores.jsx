...
const Proveedores = () => {
    ...
    useEffect(() => {
        const getProveedor = async () => {
            setLoading(true);
            try {
                const { data } = await axios.get(`https://regxipruebas.cloud/API/proveedores/all`);
    ...
    const guardarProv = async () => {
        try {
            const { data } = await axios.post(`https://regxipruebas.cloud/API/proveedores/`, proveedor);
    ...
    const ActualizaProv = async () => {
        try {
            const { data } = await axios.put(`https://regxipruebas.cloud/API/proveedores/${id_proveedor}`, proveedor);
    ...
    const deleteProv = async (id) => {
        try {
            const { data } = await axios.put(`https://regxipruebas.cloud/API/proveedores/del/${id}`);
            // const { data } = await axios.put(`http://localhost:3050/api/proveedores/del/${id}`);
            if (data.status != 200) {
                Swal.fire({
                    icon: 'error',
                    title: data.message,
